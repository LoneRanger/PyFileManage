...
def merge_tree(src, dst):
    ...
    Files in dst may already exist but will be overwritten by the src files
    whether or not they are newer. If a file already exists in dst and is not
    overwritten it will stay and not be removed. If a directory in src does not
    exist in dst it will be created and recursively filled.
    ...
    for root, dirs, files in os.walk(src):
        ...
        if not os.path.isdir(root_dst): # if the dst root doesn't exist?
            ...
            for file in files: # for files in root directory
                fqfile_src = root + "/" + file # make fully qualified name
                fqfile_dst = root_dst + "/" + file # walk returns only basename
                shutil.copy2(fqfile_src, fqfile_dst) # copy
        ...
        if root in ignore: # skip if in ignore
            ...
            for file in files:
                ...
                if file or filename in ignore: # skip if in ignore
                    continue
            if os.path.exists(filename): # protect agains broken links
                st = os.stat(filename)
                file_type = os.popen('file ' + filename).read()
                obj = (st, file_type.split()[1]) # split & use 2nd part
                stats[filename] = obj
                ## TODO:
                ## broke on filename = "51086576CM006_USA_I(2).jpg"
                ## located in /media/hdb3/kurt/images/olympic
                ## most likely by the ")" in the file's name.
            else:
                print filename, "is a broken link"
                broken_files.append(filename)
    return stats, broken_files
# ^ returns stats, list of stats & sizes, dict by size
...
def tar_entry_stats_walker(tgz):
    ...
    for taritem in tar: # go through each entry in tarball
        ...
        tmd5   = -0
        if taritem.isfile():
            shortc = short_tar_extract(tgz, tname)
            tmd5   = content_md5sum(shortc)
        tstat  = (tgz,    # Location/name of tar file
                tmd5,     # md5sum first 64K
...
src = top_a #"/media/HD-HSIU2/hdb3" #
dst = top_b #"/media/hda7/hdb3" #

print "src is %s  \ndst is %s" % (src, dst)
results = compareTrees(src, dst)
pickle_put("/home/kurt/bin/Test/compare_results.obj", results)
...
print """ In dst there are...
 %d\t missing directories
 %d\t identical files
 %d\t files with different stats
 %d\t missing files
""" % (len(dir_awol), len(file_ident), len(file_frat), len(file_awol))
n = len(results)
