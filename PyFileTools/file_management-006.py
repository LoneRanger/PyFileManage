...
def dicts_merge(d0, d1):
    ...
    for key in keys:
        if d0.has_key(key) and d1.has_key(key): # dictionaries have same key?
            ...
            print "Error, you shouldn't ever have gotten here."
        value.sort()
        newdict[key] = value
    return newdict
...
def file_tree_walker(top, ignore=[], seq=(), args=[], dict={}):
    ...
    walk_results = (norm_files,     # normal files
                    ...
                    stats)          # all files not broken with stats on each
    return walk_results
# ^ returns walk_results
...
home = "/home/oolon"
top_a = "/media/disk/kurt/Documents"
top_b = "/home/oolon/Documents"
file_data = "/home/oolon/bin/Data/File-Info"
src = top_a #"/media/HD-HSIU2/hdb3" #
dst = top_b #"/media/hda7/backup/hdb3" #

print "src is %s  \ndst is %s" % (src, dst)
#walk_results = file_tree_walker(src)
file_db = file_tree_walker(home)
for item in file_db[6]:
    shelf_add(file_data + "/homedb", item[0], item[1])
